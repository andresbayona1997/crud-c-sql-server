	...
	public class D_Productos
	{
								 ...
								 E_Productos oPro)
		{
			...
			try
			{
				...
				Comando.Parameters.Add("@mCodigo_ca", SqlDbType.Int).Value = oPro.Codigo_ca;
		...
		public string Activar_pr(int nCodigo_pr,
								 bool bEstado_activo)
		{
			string rpta = "";
			SqlConnection SqlCon = new SqlConnection();
			try
			{
				...
				SqlCon.Open();
				rpta = Comando.ExecuteNonQuery() >= 1 ? "OK" : "No se pudo cambiar el estado del producto";
			}
			catch (Exception ex)
			{
				rpta = ex.Message;
